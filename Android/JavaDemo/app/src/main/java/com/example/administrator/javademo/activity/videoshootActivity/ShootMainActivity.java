package com.example.administrator.javademo.activity.videoshootActivity;
...
import com.ruiyihong.toyshop.view.FocusSurfaceView;
import com.ruiyihong.toyshop.view.MyVideoView;
import com.ruiyihong.toyshop.view.RecordedButton;
...
import com.example.administrator.javademo.view.publish.FocusSurfaceView;
import com.example.administrator.javademo.view.publish.MyVideoView;
import com.example.administrator.javademo.view.publish.RecordedButton;
import com.yixia.camera.MediaRecorderNative;
import com.yixia.camera.VCamera;
import com.yixia.camera.model.MediaObject;
import com.yixia.videoeditor.adapter.UtilityAdapter;
...
public class ShootMainActivity extends ShootBaseActivity implements View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		maxDuration = getIntent().getIntExtra("maxDuration", 10000);
		initMediaRecorder();

		sv_ffmpeg.setTouchFocus(mMediaRecorder);

		rb_start.setMax(this.maxDuration);
	...
	private void syntVideo(){
		...
		new AsyncTask<Void, Void, String>() {
			...
			protected String doInBackground(Void... params) {
				String nowPath = AppConstants.FILE_PUBLISH_VIDEO+"/"+key;
				...
				for (int x = 0; x < mMediaObject.getMediaParts().size(); x++) {
					...
					if(cameraTypeList.get(x) == Camera.CameraInfo.CAMERA_FACING_FRONT) {
						rotateMp4 = nowPath+"/"+x+"_rotate.mp4";
				...
				for (int x = 0; x < pathList.size(); x++) {
					...
					String ts = nowPath+"/"+x+".ts";
					mp4ToTs(path, ts);
					tsList.add(ts);
				}

				output = nowPath+"/"+key+".mp4";
				boolean flag = tsToMp4(tsList, output);
				if(!flag) output = "";
				deleteDirRoom(new File(nowPath), output);
				return output;
			}
			@Override
	...
	private void initMediaRecorder() {
		...
		LogUtil.e("vcamera.getvideocachePath----"+VCamera.getVideoCachePath());
